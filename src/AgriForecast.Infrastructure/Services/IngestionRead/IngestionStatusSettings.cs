...
// Resolves the ingestion status handler's config values from the API's own configuration, applying the
// documented fallbacks. Kept at the Infrastructure boundary so the Application layer never takes a
...
public class IngestionStatusSettings : IIngestionStatusSettings
{
    ...
    public IngestionStatusSettings(IConfiguration configuration)
    {
        ...
        // Read the raw string and TryParse rather than GetValue<int?>: a non-numeric value would make GetValue
        // throw at construction (a 500 on the status endpoint). Fall back to the default on absent, non-numeric
        // or non-positive values, since a zero or negative window would make every unfinished run look crashed.
