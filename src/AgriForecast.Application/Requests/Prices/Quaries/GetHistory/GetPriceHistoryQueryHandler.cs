...
// Observed daily low-high price series for one crop, optionally one market. Owns the window clamp and the
// per-date aggregation; the DB is behind IPriceHistoryStore.
// Per calendar date, MinPrice is the lowest observed low and MaxPrice the highest observed high across
// the day's usable rows. The store already excludes quarantined rows, so nothing is fabricated here.
// An empty result is a valid 200 [], never a failure.
    ...
    : IRequestHandler<GetPriceHistoryQuery, Result<List<PriceHistoryPoint_GetDto>>>
{
        ...
        GetPriceHistoryQuery request, CancellationToken cancellationToken)
    {
        ...
        // Inclusive window of exactly windowDays calendar days ending at the latest observation.
        ...
        var points = rows
            // Fail-closed defence in depth: the store already excludes rows without a usable band, but
            // re-check so a looser store can never let a null or zero bound fabricate a range.
