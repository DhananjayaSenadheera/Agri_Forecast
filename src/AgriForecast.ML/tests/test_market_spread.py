...
Live-DB findings pinned here (verified 2026-07-04, see task report for the
raw query output):
  10 total Markets rows:
    - 4x MarketType=0 (Wholesale) with MarketCode LIKE 'ECOMAP-%' -- synthetic
      Economic-Center-Map demo/seed markets (Colombo, Nature Fresh Market,
      Green Harvest Hub, Eco Veggie Corner). EXCLUDED (ecomap rule).
    - 1x MarketType=3 (NationalAggregate) -- "CBSL national average
      (pseudo-market)", MarketCode MKT00000006. EXCLUDED (national-aggregate
      rule).
    - 5x real physical markets (feature-safe, MarketType in {0,1,2}, MarketCode
      LIKE 'MKT%'): Pettah (HARTI wholesale), Narahenpita (HARTI retail),
      Dambulla/Keppetipola/Thambuttegama Dedicated Economic Centres.
  PriceObservations coverage (LEFT JOIN COUNT) among the 5 feature-safe
  markets: Dambulla 16,802 / Pettah 16,073 / Thambuttegama 15,752 /
  Keppetipola 4,128 / Narahenpita 0. So 4 of the 5 feature-safe markets have
  real observation coverage today; Narahenpita is feature-safe by the
  exclusion rule but currently has zero rows (a coverage fact, not part of
  get_feature_safe_market_ids()'s own contract, which is name/type/code-based
  only -- pinned here for the record since the task calls it out explicitly).
"""
...
class TestFeatureSafeMarketIdsLiveContract:
    ...
    def test_excludes_every_ecomap_twin_market(self):
        ...
        assert ecomap_ids, (
            "expected at least one ECOMAP-% coded market row in the live "
            "Markets table -- test fixture assumption violated, re-verify "
            "DB seed state"
        )
