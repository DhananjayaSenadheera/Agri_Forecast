...
**P3 pre-build analysis (2026-07-04) — CBSL macro; acceptance items re-posted + extended on 86cahefby (comments 90150239063093 / 90150239067435):**
- **Corpus reality: the live CBSL path is PDF-only** (CCPI press releases + Monthly Indicators pack); Excel/openpyxl appears ONLY on the manual eResearch backfill path — scope the XXE/dimension-cap items to that path. `openpyxl` is not installed yet; pdfplumber/pdfminer.six floors already pinned (`requirements.txt:30-38`). No camelot/tabula anywhere — keep it that way (pure-Python parsers only).
- **#1 P3 regression risk: choke-point bypass.** `guarded_get` / `_download_capped` / parse-timeout are per-call-site, not global — grep any new fetcher for bare `requests.get`/`pdfplumber.open`. NCPI was CUT from scope → `statistics.gov.lk` stays OFF the allowlist (single-apex `cbsl.gov.lk` only).
- **4 approved additions beyond the 8 items:** PDF page-count cap ~50 post-open (25MB byte cap misses small-but-explosive PDFs — HARTI lacks this too); cache filenames from parsed dates, never URL basenames; SHA-256 recorded per artifact; a global FastAPI `@app.exception_handler(Exception)` backstop (today error hygiene is per-route only).
- **.NET latent trap:** the registered CBSL `HttpClient` (`InfsDependencyInjection.cs:122-128`) has no redirect hardening — harmless while `CbslPriceReportClient` throws, but if .NET ever fetches, mirror Dambulla's `AllowAutoRedirect = false`. .NET has NO role auth (bare `[Authorize]` everywhere) — ingest triggers stay off .NET controllers.

**P4 pre-build analysis (2026-07-04) — ensemble deps + artifact integrity:**
- **P4 dependency policy (owner-approved):** `prophet>=1.1.6,<1.2` + `cmdstanpy>=1.2,<2` (floor+ceiling for heavy libs; rest of requirements.txt stays floors-only); torch/tensorflow NOT added this phase (2GB, `torch.load` default-RCE); pre-P4 pip-audit baseline CLEAN (77 pkgs) — re-audit after any install.
- **Artifact rule:** everything inside the ONE signed `model.pkl` dict (Prophet via `model_to_json` string); any side-car requires sha256 in `metadata.json` + `_verify_integrity` extension — an unsigned side-car load path is a P5 HMAC hole.
- **Training guards baked in during P4:** explicit seeds for every new RNG (Prophet MAP not MCMC; torch `manual_seed`) + hard epoch/Stan-iteration caps. Wall-clock timeouts/subprocess isolation = P5.
- **Stan compilation must never run on the serving/import path** (JSON round-trip avoids recompile; payload loads at predict.py import).
